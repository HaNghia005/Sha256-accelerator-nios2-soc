...
module sha_core(
    ...
    reg [1:0] state;
    reg [6:0] round_ctr; // D�ng 7 bit c� the dem toi 64
    ...
    //  Cap t�n hieu Init v� Shift li�n tuc cho sha256_mem
    ...
    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            ...
            case (state)
                IDLE: begin
                    ...
                    if (init || next) begin
                        ...
                        if (init) begin
                            ...
                            // Reset H0-H7 khi khởi tạo thông điệp mới
                            H0 <= SHA256_H0_0; H1 <= SHA256_H0_1;
                            H2 <= SHA256_H0_2; H3 <= SHA256_H0_3;
                            H4 <= SHA256_H0_4; H5 <= SHA256_H0_5;
                            H6 <= SHA256_H0_6; H7 <= SHA256_H0_7;
                        end else begin
                ...
                CALC_WK: begin
                    // Stage 1 - Nap WK cho 2 v�ng dau ti�n (round 0, 1)
                    wk0_reg <= w0 + k0;
                    wk1_reg <= w1 + k1;
                    
                    // Look-ahead bien dem v�ng sau lay data cua round 2, 3
                    round_ctr <= 7'd2;
                    state <= CALC_ROUNDS;
                end

                CALC_ROUNDS: begin
                    // Stage 2: Cap nhat Output cua khoi logic b?m v�o thanh ghi State
                    a_reg <= a_next; b_reg <= b_next; c_reg <= c_next; d_reg <= d_next;
                    e_reg <= e_next; f_reg <= f_next; g_reg <= g_next; h_reg <= h_next;

                    // Dong thoi, Stage 1 lai tiep tuc t�nh W+K cho v�ng ke tiep
                    wk0_reg <= w0 + k0;
                    wk1_reg <= w1 + k1;
               
                    // t�nh ket qua cho W62 v� K62 d� bi tre 1 nhip do pipeline.
